...
unkown:
		...
		rc = strerror_s(buff, sizeof(buff), code);
		if (unlikely(rc))
			goto unkown;
	}
...
uint32_t byteCrc32(const void *buf, size_t size, uint32_t init)
{
    uint32_t crc = init;
...
ssize_t get_filesize(const char *path)
{
    ssize_t l;
...
ssize_t _pread(int fd, void *buf, size_t count, uint64_t offset)
{
    ...
    SetLastError(0);
    bool RF = ReadFile(file, (LPVOID)buf, (DWORD)count, &read_bytes, &overlapped);
