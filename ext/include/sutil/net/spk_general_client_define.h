...
/** 默认的UDP连接的心跳间隔(秒) */
#define GENERAL_CLI_DEFAULT_UDP_HEARTBEAT_INTERVAL      (10)
...
/**
 * 消息处理函数(OnMsg回调函数)的返回值约定
 *
 * @note 该约定并非严格的限制, 只是定义了常用的取值, 实际上可以返回大于0/等于0/小于0的任意数值
 *       - 大于等于0, 表示成功。OnMsg函数将继续运行 (不会立即返回);
 *       - 小于0, 表示失败。OnMsg函数将中止运行立即返回。
 */
typedef enum {
    GENERAL_CLI_RTCODE_SUCCESS              = 0,        /**< 大于等于0, 表示处理成功 */
    GENERAL_CLI_RTCODE_ERROR                = -1,       /**< 小于0, 表示处理失败 */
    GENERAL_CLI_RTCODE_BREAK                = INT_MIN   /**< 特殊的小于0的数, 可用于中断OnMsg函数, 使其立即返回 */
} eGeneralClientReturnCodeT;
/* -------------------------           */
...
typedef struct _SGeneralClientChannel {
    ...
    /** 累计接收到的未压缩数据大小 */
    int64               __totalInMsgSize;
    /** 累计接收到的已压缩数据大小 */
    int64               __totalCompressedSize;
    /** 解压缩后的数据总大小 */
    int64               __totalDecompressSize;
    ...
    int32               __clientId;         /**< 客户端编号 */
    uint8               __channelType;      /**< 通道类型 */
    int8                __clEnvId;          /**< 客户端环境号 */
    uint8               __groupFlag;        /**< 通道组标志 */
    uint8               __protocolHints;    /**< 协议约定信息 */
    uint8               __businessType;     /**< 通道对应的业务类型 */
    /** 最近一次连接的主机地址顺序号 (顺序号从1开始) */
    uint8               __lastConnectAddrIdx;
...
typedef struct _SGeneralClientRemoteCfg {
    ...
    uint8               clusterType;        /**< 服务器集群的集群类型 (0:默认, 1:复制集, 2:对等节点) */
    int8                clEnvId;            /**< 客户端环境号 */
    uint8               targetSetNum;       /**< 远程主机的集群号 */
    uint8               businessType;       /**< 期望对接的业务类型 */
