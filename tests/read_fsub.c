...
int main(int argc, char **argv)
{
    ...
    if (argc < 4) {
out:
        fprintf(stderr, "./read_file <tcp://ip:port> <dir> <topic> [topic ...]"
                "\n");
        exit(EXIT_FAILURE);
    }

    struct mds_fsub *sub = mds_fsub_open(argv[1], argv[2]);
    if (!sub) {
    ...
    do {
        ...
        rc = mds_fsub_recv(sub, &data, 5000);
        if (rc < 0) {
            if (rc == -ETIMEDOUT)
                continue;
            break;
